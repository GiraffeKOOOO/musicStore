...
<head>
	...
	<link rel="stylesheet" type="text/css" href="../css/categories/accessories.css">
</head>
<body>
	...
	<div id="main-div">
			...
			<div id="top-bar-2">
				<table id="menu-bar">
					<tr id="row-one">
						...
						<td id="td-specific">Accessories</td>
					</tr>
				</table>
			</div>
		</div>
		...
		<div id="accessories-products">
			<p id="accessories-title"><u>Accessories</u></p>

			<?php
			include '../php/script-db.php';

			$query_accessories = "SELECT * FROM products WHERE category='Accessories'";

			$accessories_id = array();
			$accessories_name = array();
			$accessories_price = array();
			$stock = array();
			$accessories_picture = array();

			$result = mysqli_query($conn, $query_accessories);
			$divCounter = mysqli_num_rows($result);

			while ($row = mysqli_fetch_array($result)) {
				$accessories_id[] = $row['product_ID'];
				$accessories_name[] = $row['product_name'];
				$accessories_price[] = $row['product_price'];
				$stock[] = $row['stock_quantity'];
				$accessories_picture[] = $row['product_image'];
			}
			for ($i=0; $i < $divCounter; $i++) { 
			echo "<div class='accessories-product'>
			<span>
			<table class='accessories-product-table'>
				<tr>
					<td>Product name: </td>
					<td>$accessories_name[$i]</td>
				</tr>

				<tr>
					<td>Product price: </td>
					<td>£$accessories_price[$i]</td>
				</tr>
				...
				<span>
				<a href='../category/product.php?pid=$accessories_id[$i]'>
				<img src='../media/$accessories_picture[$i]' class='accessories-picture'>
				</a>
				</span>
