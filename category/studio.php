...
<head>
	<title>Studio</title>
	<link rel="stylesheet" type="text/css" href="../css/categories/studio.css">
...
<body>
	...
	<div id="main-div">
		...
		<div id="studio-products">
			<p id="studio-title"><u>Studio Equipment</u></p>

			<?php
			include '../php/script-db.php';

			$query_studio = "SELECT * FROM products WHERE category='Studio'";
			...
			$result = mysqli_query($conn, $query_studio);
			$divCounter = mysqli_num_rows($result);

			while ($row = mysqli_fetch_array($result)) {
				$studio_id[] = $row['product_ID'];
				$studio_name[] = $row['product_name'];
				$studio_price[] = $row['product_price'];
				$stock[] = $row['stock_quantity'];
				$studio_picture[] = $row['product_image'];
			}
			for ($i=0; $i < $divCounter; $i++) { 
			echo "<div class='studio-product'>
			<span>
			<table class='studio-product-table'>
				<tr>
					<td>Product name: </td>
					<td>$studio_name[$i]</td>
				</tr>
				...
				<span>
				<a href='../category/product.php?pid=$studio_id[$i]'>
				<img src='../media/$studio_picture[$i]' class='studio-picture'>
				</a>
				</span>
