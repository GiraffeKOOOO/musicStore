...
<head>
	<title>Strings</title>
	<link rel="stylesheet" type="text/css" href="../css/categories/strings.css">
...
<body>
	...
	<div id="main-div">
		...
		<div id="strings-products">
			<p id="strings-title"><u>Strings</u></p>

			<?php
			include '../php/script-db.php';

			$query_strings = "SELECT * FROM products WHERE category='Strings'";

			$strings_id = array();
			$strings_name = array();
			$strings_price = array();
			$stock = array();
			$strings_picture = array();

			$result = mysqli_query($conn, $query_strings);
			$divCounter = mysqli_num_rows($result);

			while ($row = mysqli_fetch_array($result)) {
				$strings_id[] = $row['product_ID'];
				$strings_name[] = $row['product_name'];
				$strings_price[] = $row['product_price'];
				$stock[] = $row['stock_quantity'];
				$strings_picture[] = $row['product_image'];
			}
			for ($i=0; $i < $divCounter; $i++) { 
			echo "<div class='strings-product'>
			<span>
			<table class='strings-product-table'>
				<tr>
					<td>Product name: </td>
					<td>$strings_name[$i]</td>
				</tr>

				<tr>
					<td>Product price: </td>
					<td>£$strings_price[$i]</td>
				</tr>
				...
				<span>
				<a href='../category/product.php?pid=$strings_id[$i]'>
				<img src='../media/$strings_picture[$i]' class='strings-picture'>
				</a>
				</span>
