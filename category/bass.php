...
<head>
	<title>Bass</title>
	<link rel="stylesheet" type="text/css" href="../css/categories/bass.css">
...
<body>
	...
	<div id="main-div">
		...
		<div id="bass-products">
			<p id="bass-title"><u>Bass</u></p>

			<?php
			include '../php/script-db.php';

			$query_bass = "SELECT * FROM products WHERE category='Bass'";
			...
			$result = mysqli_query($conn, $query_bass);
			$divCounter = mysqli_num_rows($result);

			while ($row = mysqli_fetch_array($result)) {
				$bass_id[] = $row['product_ID'];
				$bass_name[] = $row['product_name'];
				$bass_price[] = $row['product_price'];
				$stock[] = $row['stock_quantity'];
				$bass_picture[] = $row['product_image'];
			}
			for ($i=0; $i < $divCounter; $i++) { 
			echo "<div class='bass-product'>
			<span>
			<table class='bass-product-table'>
				<tr>
					<td>Product name: </td>
					<td>$bass_name[$i]</td>
				</tr>
				...
				<span>
				<a href='../category/product.php?pid=$bass_id[$i]'>
				<img src='../media/$bass_picture[$i]' class='bass-picture'>
				</a>
				</span>
