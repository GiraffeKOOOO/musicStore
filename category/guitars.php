...
<head>
	<title>Guitar</title>
	<link rel="stylesheet" type="text/css" href="../css/categories/guitars.css">
...
<body>
	...
	<div id="main-div">
		...
		<div id="guitars-products">
			<p id="guitars-title"><u>Guitars</u></p>

			<?php
			include '../php/script-db.php';

			$query_guitars = "SELECT * FROM products WHERE category='Guitar'";

			$guitar_id = array();
			$guitar_name = array();
			$guitar_price = array();
			$stock = array();
			$guitar_picture = array();

			$result = mysqli_query($conn, $query_guitars);
			$divCounter = mysqli_num_rows($result);

			while ($row = mysqli_fetch_array($result)) {
				$guitar_id[] = $row['product_ID'];
				$guitar_name[] = $row['product_name'];
				$guitar_price[] = $row['product_price'];
				$stock[] = $row['stock_quantity'];
				$guitar_picture[] = $row['product_image'];
			}
			for ($i=0; $i < $divCounter; $i++) { 
			echo "<div class='guitar-product'>
			<span>
			<table class='guitar-product-table'>
				<tr>
					<td>Product name: </td>
					<td>$guitar_name[$i]</td>
				</tr>
				...
				<span>
				<a href='../category/product.php?pid=$guitar_id[$i]'>
				<img src='../media/$guitar_picture[$i]' class='guitar-picture'>
				</a>
				</span>
