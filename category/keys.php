...
<head>
	<title>Keys</title>
	<link rel="stylesheet" type="text/css" href="../css/categories/keys.css">
...
<body>
	...
	<div id="main-div">
		...
		<div id="keys-products">
			<p id="keys-title"><u>Keys</u></p>

			<?php
			include '../php/script-db.php';

			$query_keys = "SELECT * FROM products WHERE category='Keys'";
			...
			$result = mysqli_query($conn, $query_keys);
			$divCounter = mysqli_num_rows($result);

			while ($row = mysqli_fetch_array($result)) {
				$keys_id[] = $row['product_ID'];
				$keys_name[] = $row['product_name'];
				$keys_price[] = $row['product_price'];
				$stock[] = $row['stock_quantity'];
				$keys_picture[] = $row['product_image'];
			}
			for ($i=0; $i < $divCounter; $i++) { 
			echo "<div class='keys-product'>
			<span>
			<table class='keys-product-table'>
				<tr>
					<td>Product name: </td>
					<td>$keys_name[$i]</td>
				</tr>
				...
				<span>
				<a href='../category/product.php?pid=$keys_id[$i]'>
				<img src='../media/$keys_picture[$i]' class='keys-picture'>
				</a>
				</span>
